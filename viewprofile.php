...
if (!isset($_GET['user_id'])) {
    $query = "SELECT username, first_name, last_name, gender, birthdate, city, state, picture FROM mismatch_user WHERE id = '" .  $_COOKIE['user_id'] .  "'";
}
else {
    $query = "SELECT username, first_name, last_name, gender, birthdate, city, state, picture FROM mismatch_user WHERE id = '" .  $_GET["user_id"] .  "'";
}
...
if (mysqli_num_rows($data) == 1) {
    ...
    if (!empty($row['picture'])) {
        echo '<tr><td class="label">Picture:</td><td><img src="' . GW_UPLOADPATH . $row['picture'] .
